#include "imgui.h"

#define ITEM_DISABLED ImVec4(0.50f, 0.50f, 0.50f, 1.00f)
#define ITEM_UNAVAILABLE ImVec4(0.0f, 0.501f, 0.0f, 1.00f)  // Green color
#define BLACK ImVec4(0.05f, 0.05f, 0.05f, 1.0f)
#define WHITE ImVec4(1.0f, 1.0f, 1.0f, 1.0f)
#define BLACK_TRANSPARENT ImVec4(0.05f, 0.05f, 0.05f, 0.7f)
#define PURPLE ImVec4(0.0f, 0.501f, 0.0f, 1.0f)  // Green color
#define MAGENTA ImVec4(0.0f, 0.501f, 0.0f, 1.0f)  // Green color
#define LOG_OK ImVec4(0.50f, 0.77f, 0.14f, 1.0f)
#define LOG_ERROR ImVec4(0.85f, 0.03f, 0.09f, 1.0f)
